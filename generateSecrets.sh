...
# edit by sanal
if [ -s ./policy1.xml ]; then
  SECRET_ARGS="${SECRET_ARGS} --from-file=policy1=./policy1.xml "
else
  SECRET_ARGS="${SECRET_ARGS} --from-literal=policy1="
fi
# 2nd edit for replay policy by sanal
if [ -s ./policyreplay.xml ]; then
  SECRET_ARGS="${SECRET_ARGS} --from-file=policyreplay=./policyreplay.xml "
else
  SECRET_ARGS="${SECRET_ARGS} --from-literal=policyreplay="
fi
